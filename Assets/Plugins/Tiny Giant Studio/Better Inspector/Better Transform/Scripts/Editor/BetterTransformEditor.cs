            ...
            if (meshFilters.Length > 0)
            {
                ...
                foreach (MeshFilter meshFilter in meshFilters)
                {
                    ...
                    foreach (Vector3 vertex in vertices)
                    {
                        ...
                        Vector3 worldVertex = meshTransform.localToWorldMatrix.MultiplyPoint3x4(vertex);

                        // Convert world space vertex back to target's local space
                        Vector3 localVertex = target.transform.worldToLocalMatrix.MultiplyPoint3x4(worldVertex);
